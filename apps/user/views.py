...
class LoginView(View):
    ...
    def post(self, request):
        ...
        if login_form.is_valid():
            ...
            catagory = login_form.cleaned_data.get('catagory')
            # 成功返回user对象,失败None
...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            catagory = register_form.cleaned_data.get('catagory')
            # 如果用户已存在，则提示错误信息
            if UserProfile.objects.filter(username=user_name):
                return render(request, 'register.html', {'register_form': register_form, 'msg': '用户已存在'})
                # 两次密码输入不一致，则提示错误信息
            if pass_word != re_password:
                return render(request, 'register.html', {'register_form': register_form, 'msg': '密码不一致！'})
            # 实例化一个user_profile对象
            ...
            user_profile.category = catagory
            user_profile.save()
...
class UpdatePwdView(View):
    ...
    def get(self, request):
        ...
        return render(request, 'updata_pwd.html')
    ...
    def post(self, request):
        ...
        if modify_form.is_valid():
            ...
            pwd2 = request.POST.get("password2", "")
            if pwd1 != pwd2:
                return render(request, "updata_pwd.html", { "msg": "密码不一致！"})
            user = UserProfile()
            user.password = make_password(pwd2)
            user.save()
            return redirect('/login/')
        else:
            return render(request, "updata_pwd.html", {"modify_form": modify_form})
